...
async function fetchSessionEvents(sessionIds: string[]): Promise<RRWebEvent[]> {
...
async function fetchDOMSnapshots(sessionIds: string[]) {
  const query = await clickhouse.query({
    query: `
...
export const initRRWebGroupAnalysis = async () => {
  ...
  const analysisPrompt = PromptTemplate.fromTemplate(`
    Analyze the following user sessions recording summary and page content:
